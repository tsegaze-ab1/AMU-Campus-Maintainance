...
function default_categories_for_presentation(): array
{
    return [
        '🔌 Electrical Issues',
...
// Ensures the default categories exist for a clean demo.
// Safe to call multiple times (uses INSERT IGNORE + unique name).
function ensure_default_categories_present(): void
{
    global $conn;

    $insertSql = 'INSERT IGNORE INTO categories (name) VALUES (?)';
    $insert = $conn->prepare($insertSql);
    foreach (default_categories_for_presentation() as $name) {

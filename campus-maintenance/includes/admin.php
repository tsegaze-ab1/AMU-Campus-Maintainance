...
{
    // Presentation scope: only three roles.
    return ['student', 'technician', 'admin'];
}
...
function list_technicians_and_staff(): array
{
    ...
    // Only technicians can be assigned work in the simplified role model.
    $sql = "SELECT id, username, email, role FROM users WHERE role IN ('technician') ORDER BY username ASC, email ASC";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
